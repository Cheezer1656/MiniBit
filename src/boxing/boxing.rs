...
struct CombatQuery {
    ...
    gamestate: &'static mut PlayerGameState,
}
...
fn handle_combat_events(
    ...
    for mut player in clients.iter_mut() {
        if losers.contains_key(&player.gamestate.game_id) {
            let Some(loser) = losers.get(&player.gamestate.game_id) else {
                continue;
            };
            player.client.send_chat_message(format!(
                "{} died! Btw you have {} wins!",
                usernames
                    .get(*loser)
                    .unwrap_or(&Username(String::from("Unknown"))),
                player.gamestate.wins + if *loser == player.entity { 0 } else { 1 }
            ));
            end_game.send(EndGameEvent {
